...
image_preview_160x200 = Image.new("RGB", (160, 200), "black")
...
#parse koala 
    temp_8byte=[]
    my_start=2  #skip startaddress
    my_size=8000
    for i in range( my_start, my_start+my_size ):
        temp = struct.unpack('B',data_in[i])
        temp_8byte.append(temp[0])
    #fill koala_bitmap 1000*8bytes with data
    for i in range( 0, 1000 ):
        koala_bitmap.append(temp_8byte[i*8:(i+1)*8])

    my_start += my_size
    my_size = 1000
    for i in range( my_start, my_start+my_size ):
        temp = struct.unpack('B',data_in[i])
        koala_col12.append(temp[0])

    my_start += my_size
    my_size = 1000
    for i in range( my_start, my_start+my_size ):
        temp = struct.unpack('B',data_in[i])
        koala_col3.append(temp[0])

    my_start += my_size
    koala_bg = struct.unpack('B',data_in[my_start])
...
def convert_to_koala(
    ...
    * sets: koala_bitmap, koala_col12, koala_col3 and koala_bg
    ...
    my_list = list(image_preview_160x200.getdata()) #image is in "P" mode
    for y in range(0,200) :
        for x in range(0,160) :
...
def image_koala_create(
) :
    """
    creates the koala image object
    """
    convert_to_koala()    #reads: image_preview_160x200 sets koala_bitmap, koala_col12, koala_col3 and koala_bg
    koala_to_image()    #prepares koala_preview_image_data
    my_image = Image.fromarray(koala_preview_image_data, 'RGB')
    my_image = my_image.resize((320,200))
...
#http://pillow.readthedocs.io/en/latest/reference/Image.html#PIL.Image.size
    my_image = my_image.resize((160,200))
...
def action_image_refresh():
    ...
    if (user_modes.get() == "palette") :
        ...
        image_preview_160x200 = image_preview_create_effects(image_grayscale)
        image_preview_160x200 = image_quantize_paletted_brightness(image_preview_160x200)
        image_preview_160x200 = image_preview_160x200.convert("RGB")
        image_preview_160x200 = image_quantize_c64_colors(image_preview_160x200)
    else:
        image_preview_160x200 = image_preview_create_effects(image_original)
        image_preview_160x200 = image_quantize_c64_colors(image_preview_160x200)

    image_preview = image_preview_160x200.resize((320,200))
...
def action_SaveFile():
    ...
    try:
        var_start_address = int (user_start_address.get())
    except ValueError:
        var_start_address = 0
